...
def index(domain=None):
    ...
    # Count GIs for each genome
    for seq_id, data in seq_map.items():
        out_files = [f for f in data.get("files", []) if f.endswith(".out")]
        gi_count = 0
        last_update = "N/A"
        for out_file in out_files:
            filepath = os.path.join(OUTPUT_DIR, out_file)
            if os.path.exists(filepath):
                gi_count += sum(1 for line in open(filepath) if line.startswith("<GI>"))
                last_update = get_last_updated(filepath)
